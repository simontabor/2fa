...
// get a base36 crypto secure key
     ...
     * @returns A key you can store, check codes and generate QR codes from.
     * @example
...
TFA.generateKey = function(length, cb) {
  ...
  get();
     ...
     * let validCode = tfa.veryifyHOTP(key, code, counter);
     * if (validCode == true) {
     * //Log user in
     * } else {
...
TFA.verifyHOTP = function(key, code, counter, opts) {
  ...
  return false;
     ...
     * @returns Either true or false depending on if the code is valid or not.
     * @example
     * let validCode = tfa.verifyTOTP(key, code);
     * if (validCode == true) {
     * //Log user in
     * } else {
     * //Display erorr message
...
TFA.generateCode = function(key, counter, opts) {
  ...
  return ourCode;
     ...
     * @example
     * let multikey = tfa.base32Encode(key);
...
TFA.generateGoogleQR = function(name, account, key, opts, cb) {
  ...
  pngStream.on('end', function() {
     ...
     * @param pattern The pattern you want the codes to be generated in
     * @returns An array of backup codes you can store and show the user
     * @example
